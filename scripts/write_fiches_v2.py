...
const allContent: ContentItem[] = [
  ...
  },
  {
    ...
    tags: ["Agriculture", "Alimentation", "EPCI"], readTime: "15 min", icon: "\\ud83c\\udf3e",
  },
  {
    id: "ecolieux",
    ...
    tags: ["Habitat", "Innovation", "Territoire"], readTime: "8 min", icon: "\\ud83c\\udfe1",
  },
  {
    ...
    tags: ["\\u00c9conomie", "Environnement", "Strat\\u00e9gie"], readTime: "11 min", icon: "\\ud83d\\udcca",
  },
  {
  ...
  },
  {
    ...
    tags: ["Finances", "Budget vert", "Investissement"], icon: "\\ud83d\\udcb0", source: "I4CE", date: "Nov. 2025",
  },
  {
    ...
    tags: ["Politique publique", "Guide", "R\\u00e9f\\u00e9rence"], icon: "\\ud83d\\udcda", source: "Solutions Transitions", date: "Oct. 2025",
  },
  {
    id: "podcast-fabrique",
    title: "Le Podcast de la Fabrique des Transitions",
    type: "ressource", theme: "Pilotage & m\\u00e9thode", format: "Podcast",
    description: "Des \\u00e9changes inspirants avec les acteurs de terrain qui font avancer la transition \\u00e9cologique.",
    tags: ["Podcast", "T\\u00e9moignages", "Inspiration"], icon: "\\ud83c\\udf99\\ufe0f", source: "La Fabrique des Transitions", date: "Continu",
  },
  {
...
const themes = [
  ...
  { name: "Mobilit\\u00e9",                 icon: Bike,       color: "#0ea5e9", light: "#e0f2fe", border: "#bae6fd" },
  { name: "Agriculture & alimentation",    icon: Sprout,     color: "#84cc16", light: "#f7fee7", border: "#d9f99d" },
  { name: "Biodiversit\\u00e9 & nature",    icon: TreePine,   color: "#10b981", light: "#d1fae5", border: "#a7f3d0" },
  ...
  { name: "Cadre l\\u00e9gal",             icon: Scale,      color: "#64748b", light: "#f1f5f9", border: "#e2e8f0" },
  { name: "Pilotage & m\\u00e9thode",      icon: LayoutGrid, color: "#14b8a6", light: "#ccfbf1", border: "#99f6e4" },
] as const;
...
const formatLabels: Record<string, { label: string; icon: React.ElementType }> = {
  "Rapport":  { label: "Rapport",  icon: FileText },
  "Guide":    { label: "Guide",    icon: BookOpen },
  "\\u00c9tude":   { label: "\\u00c9tude",   icon: BarChart3 },
  "Podcast":  { label: "Podcast",  icon: Headphones },
  "Analyse":  { label: "Analyse",  icon: BarChart3 },
};
...
function Card({ item }: { item: ContentItem }) {
  ...
  const fmt   = item.format ? formatLabels[item.format] : null;

  return (
    <Link href={href} className="group block h-full">
      <div
        className="h-full flex flex-col bg-white rounded-2xl border border-slate-100 overflow-hidden
      ...
      >
        ...
        <div className="px-5 pt-5 pb-4">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            >
              {isFiche ? "Fiche pratique" : (fmt?.label ?? "Ressource")}
            </span>
          ...
          {/* Title */}
          <h3 className="font-bold text-slate-900 text-[15px] leading-snug line-clamp-3
    ...
    <section>
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          >
            <theme.icon className="w-4.5 h-4.5" style={{ color: theme.color }} />
          </div>
          <div>
        ...
        <button
          onClick={onFilter}
          className="flex items-center gap-1 text-[13px] font-semibold text-slate-400
...
// ─── Hub inner (uses useSearchParams) ──────────────────────────────────────────
function FichesHub() {
  ...
  // React to URL changes (navbar clicks, back/forward)
  useEffect(() => {
    const t = searchParams.get("type");
  ...
  const resetAll = () => {
    setSearch(""); setTypeFilter("tout"); setThemeFilter(""); setFormatFilter("");
  };

  const handleType = (t: TypeFilter) => {
    setTypeFilter(t);
    ...
    <div className="min-h-screen bg-slate-50">
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              <span className="text-[11px] font-bold uppercase tracking-widest text-emerald-600
                               bg-emerald-50 border border-emerald-100 px-3 py-1.5 rounded-full">
                Hub de contenus
              </span>
            </div>
            ...
            <p className="text-slate-500 text-base max-w-xl">
              {allContent.filter(i => i.type === "fiche").length} fiches pratiques et{" "}
              {allContent.filter(i => i.type === "ressource").length} ressources s\\u00e9lectionn\\u00e9es
              pour acc\\u00e9l\\u00e9rer la transition dans votre territoire.
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="sticky top-[64px] lg:top-[80px] z-30 bg-white border-b border-slate-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          ...
          <div className="flex gap-3 mb-3">
            ...
            <div className="relative flex-1 min-w-0">
              ...
              <input
                ...
                className="w-full h-10 pl-10 pr-9 text-sm bg-slate-50 border border-slate-200
            ...
            <div className="flex h-10 bg-slate-100 rounded-xl p-1 shrink-0">
              {(["tout", "fiche", "ressource"] as const).map((t) => {
                const labels: Record<typeof t, { label: string; icon: React.ElementType }> = {
                  tout:      { label: "Tout",       icon: BookOpen },
                  fiche:     { label: "Fiches",     icon: FileText },
                  ressource: { label: "Ressources", icon: Library },
                };
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3.5 rounded-lg text-sm font-semibold
          ...
          {/* Row 2 — Theme chips + Format (when Ressources) */}
          <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">

            {/* Format filters (only for Ressources tab) */}
            {typeFilter === "ressource" && (
              <>
                {["Rapport", "Guide", "\\u00c9tude", "Podcast", "Analyse"].map((f) => {
                  const active = formatFilter === f;
                  return (
                    <button
                      key={f}
                      onClick={() => setFormatFilter(active ? "" : f)}
                      className={`flex items-center gap-1.5 h-7 px-3 rounded-full text-xs font-semibold
            ...
            {themes.map((th) => {
              const count  = allContent.filter(
                (i) => i.theme === th.name && (typeFilter === "tout" || i.type === typeFilter)
              ).length;
              ...
              return (
                <button
                  ...
                  style={
                    active
                      ? { background: th.color, borderColor: th.color }
                      : { color: th.color }
                  }
                >
                  <th.icon className="w-3 h-3" />
              ...
              >
                <div className="flex items-center gap-2 flex-wrap pt-2.5">
                  <span className="text-xs text-slate-500">
                    <span className="font-bold text-slate-800">{filtered.length}</span> r\\u00e9sultat{filtered.length > 1 ? "s" : ""}
                  ...
                  {typeFilter !== "tout" && (
                    <span className="inline-flex items-center gap-1 text-xs bg-emerald-50 text-emerald-700
                      ...
                      <button onClick={() => handleType("tout")} className="hover:text-emerald-900 leading-none">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  )}
                  {themeFilter && (
                    <span className="inline-flex items-center gap-1 text-xs bg-slate-100 text-slate-700
                                     border border-slate-200 px-2 py-0.5 rounded-full font-semibold">
                      {themeFilter}
                      <button onClick={() => setThemeFilter("")} className="hover:text-slate-900 leading-none">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  )}
                  {formatFilter && (
                    <span className="inline-flex items-center gap-1 text-xs bg-slate-100 text-slate-700
                                     border border-slate-200 px-2 py-0.5 rounded-full font-semibold">
                      {formatFilter}
                      <button onClick={() => setFormatFilter("")} className="hover:text-slate-900 leading-none">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  )}
                  {search && (
                    <span className="inline-flex items-center gap-1 text-xs bg-slate-100 text-slate-700
                                     border border-slate-200 px-2 py-0.5 rounded-full font-semibold">
                      \\u201c{search}\\u201d
                      <button onClick={() => setSearch("")} className="hover:text-slate-900 leading-none">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  )}
                  ...
                  {activeFilterCount > 1 && (
                    <button
                      onClick={resetAll}
                      className="text-xs text-slate-400 hover:text-slate-700 font-semibold transition-colors underline underline-offset-2"
                    >
                      Tout effacer
                    </button>
                  )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <AnimatePresence mode="wait">
          ...
          {/* ── Browse mode: sections par th\\u00e9matique ── */}
          {!isFiltered ? (
            <motion.div
              key="browse"
              initial={{ opacity: 0 }}
              ...
              {themes.map((th) => {
                ...
                return (
                  <ThemeSection
                    ...
                    theme={th}
                    items={items}
                    onFilter={() => setThemeFilter(th.name)}
                  />
                );
              })}
            </motion.div>
            ...
            >
              ...
              <p className="text-slate-500 mb-7 max-w-sm mx-auto text-sm">
                Essayez de modifier vos crit\\u00e8res ou r\\u00e9initialisez les filtres.
              </p>
              <button
                onClick={resetAll}
                className="inline-flex items-center gap-2 px-6 py-2.5 bg-emerald-600 text-white
                           text-sm font-semibold rounded-xl hover:bg-emerald-700 transition-colors"
              >
            ...
            /* ── Filtered grid ── */
            <motion.div
              key={`grid-${typeFilter}-${themeFilter}-${formatFilter}`}
