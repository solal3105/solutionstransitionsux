...
# Regular string (not f-string) — single braces are written as-is, no escaping needed
...
lines = TSX.count("\n")
print(f"OK — {len(TSX)} chars, {lines} lines written")

import re
bad = re.findall(r'\\u[0-9a-fA-F]{4}', TSX)
if bad:
    print(f"WARNING: {len(bad)} literal \\u sequences found: {bad[:5]}")
else:
    print("Clean: no literal \\u sequences")
